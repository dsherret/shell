...
import { cpCommand, mvCommand } from "./commands/cpMv.ts";
    ...
    return this.#newWithState((state) => {
      if (reader === "inherit" || reader === "null") {
        ...
        state.stdin = new Deferred(async () => {
          ...
          if (!(body?.readable instanceof ReadableStream)) {
            throw new TypeError(
              "When passing an awaitable to stdin, the resolved value must have a `readable` ReadableStream.",
            );
          }
...
export function template(strings: TemplateStringsArray, exprs: TemplateExpr[]): CommandBuilderStateCommand {
  return templateInner(strings, exprs, escapeArg);
}

export function templateRaw(strings: TemplateStringsArray, exprs: TemplateExpr[]): CommandBuilderStateCommand {
...
function isAwaitableReadable(value: unknown): value is AwaitableReadable {
  return value != null
